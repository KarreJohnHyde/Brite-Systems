...
5. Choose exactly one decision:
   - ANSWER only when the supplied excerpts explicitly and completely settle the material question and are internally consistent.
   - CONFLICT when supplied excerpts materially disagree and their scopes do not resolve the disagreement.
   - REFUSE when evidence is absent, partial, merely related, ambiguous, missing a material condition, or otherwise insufficient.
...
8. Keep the answer concise and faithful to the excerpts. Preserve every material condition and exception.
9. Return only JSON matching this object shape:
